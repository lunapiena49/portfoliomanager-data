# Termini di Servizio -- Portfolio Manager by PluriFin

**Versione**: 1.1.0
**Ultimo aggiornamento**: 2026-08-15
**Lingua originale**: Italiano (versione di riferimento per il foro competente)

---

## 1. Parti del contratto

I presenti Termini di Servizio (ToS) regolano l'uso dell'applicazione
**Portfolio Manager** (di seguito "l'App") fornita da:

- **Filippo Salemi** -- persona fisica, sviluppatore indipendente
- **Brand commerciale**: PluriFin
- **Paese**: Italia
- **Email contatto**: `info@plurifin.app`

L'utente che installa, avvia o utilizza l'App ("l'Utente") accetta integralmente
i presenti termini. Se non li accetta, l'Utente deve disinstallare l'App e
cessarne l'uso.

## 2. Oggetto del servizio

L'App e' uno strumento informativo per la gestione personale del proprio
portafoglio di investimenti, che permette di:

- Importare estratti CSV o PDF da broker supportati
- Inserire e tenere traccia manualmente di posizioni
- Definire obiettivi finanziari personali
- Visualizzare dati di mercato pubblici
- Richiedere analisi descrittive da modelli AI di terze parti (opzionale)
- Calcolare suggerimenti di ribilanciamento basati su target user-defined

L'App e' un **software**, non un servizio finanziario, non un broker, non un
consulente, non un intermediario. Vedi anche `disclaimer_financial.md` (parte
integrante del contratto).

## 3. Cosa l'App NON e' e NON fa

Per evitare ogni ambiguita':

- **NON e' un servizio di consulenza finanziaria** ai sensi dell'art. 1 comma
  5-septies del TUF (D.Lgs. 24 febbraio 1998 n. 58)
- **NON e' un servizio di gestione di portafogli** ai sensi dell'art. 1 comma
  5 lettera d) del TUF
- **NON e' un servizio di intermediazione, esecuzione ordini, ricezione e
  trasmissione di ordini** ai sensi del TUF
- **NON e' un servizio MiFID II** (Direttiva 2014/65/UE)
- **NON esegue, propone o suggerisce alcuna operazione di acquisto, vendita o
  scambio di strumenti finanziari**
- **NON detiene fondi o strumenti finanziari dell'Utente**
- **NON ha rapporti con broker, banche, intermediari**: l'Utente importa i
  propri dati manualmente o tramite file di estratto autoprodotto
- **NON garantisce alcun rendimento**, alcuna performance, alcun risultato
  economico
- **NON e' soggetta a vigilanza CONSOB ne' Banca d'Italia** in quanto non
  fornisce servizi di investimento riservati

L'App e' equiparabile, sotto il profilo regolamentare, a un foglio di calcolo
arricchito: traccia, organizza, mostra. Le decisioni restano all'Utente.

## 4. Modello commerciale e prezzi

### 4.1 Versione free, trial e versioni Premium

PluriFin offre una versione free, un trial gratuito e quattro varianti
Premium (Annual/Lifetime x Single/Family). Tutte le varianti Premium
sbloccano lo stesso insieme di funzionalita': la differenza riguarda il
modello di pagamento (ricorrente annuale vs una tantum) e il numero di
account autorizzati (1 account Google vs fino a 6 account collegati a un
Google Family Group).

| Versione | Costo | Modello | Funzionalita' |
|---|---|---|---|
| Web demo | Gratuita | n/a | Max 5 posizioni manuali, no import CSV/PDF, mercato weekly, obiettivi locked, AI locked |
| Trial Android (7 giorni) | Gratuita, **senza richiesta carta upfront** | n/a | Tutte le funzionalita' Premium attive per 7 giorni dal primo avvio |
| Premium Single Annual | EUR 4,99 / anno | Rinnovo automatico annuale | Tutte le funzionalita' attive, singolo account Google |
| Premium Single Lifetime | EUR 29,99 una tantum | Pagamento unico, **nessun rinnovo** | Tutte le funzionalita' attive a vita, singolo account Google |
| Premium Family Annual | EUR 9,99 / anno | Rinnovo automatico annuale | Tutte le funzionalita' attive per Account Holder + fino a 5 Family Members del medesimo Google Family Group |
| Premium Family Lifetime | EUR 49,99 una tantum | Pagamento unico, **nessun rinnovo** | Tutte le funzionalita' attive a vita per Account Holder + fino a 5 Family Members del medesimo Google Family Group |

I prezzi sono definiti in EUR per il mercato italiano. Per altri paesi
Google Play applica una conversione automatica con parita' di potere
d'acquisto. L'IVA, ove dovuta, e' inclusa nel prezzo finale mostrato da
Google Play al momento dell'acquisto.

### 4.2 Modalita' di pagamento

L'unico canale di pagamento attivo nella v1.0 e' **Google Play Billing**, che
supporta: Google Pay, carte di credito/debito (tutti i circuiti supportati
da Google Play in Italia), addebito su saldo Google Play, gift card.

Il pagamento e' gestito **interamente da Google**: PluriFin non riceve dati di
carta, IBAN o credenziali di pagamento.

### 4.3 Tipologie di abbonamento Premium

Le quattro varianti Premium si distinguono per due assi indipendenti.

- **Annual** (Single Annual, Family Annual): contratto ad esecuzione
  continuata con rinnovo automatico annuale. Il pagamento ricorrente e'
  disciplinato dalla sezione 4.4 (Rinnovo automatico) e dalla sezione 4.7
  (Variazione prezzo).
- **Lifetime** (Single Lifetime, Family Lifetime): contratto ad esecuzione
  istantanea con pagamento unico. Non comporta alcun rinnovo automatico
  ne' addebito periodico. L'accesso alle funzionalita' Premium e'
  garantito a vita, fatte salve le ipotesi di cessazione del servizio
  (sezione 10.3).
- **Single** (Single Annual, Single Lifetime): abilita le funzionalita'
  Premium per un singolo account Google.
- **Family** (Family Annual, Family Lifetime): abilita le funzionalita'
  Premium per l'Account Holder che effettua l'acquisto e per i Family
  Members facenti parte del medesimo Google Family Group, alle condizioni
  della sezione 4.6.

### 4.4 Rinnovo automatico (solo abbonamenti Annual)

Gli abbonamenti **Single Annual** e **Family Annual** si rinnovano
automaticamente ogni 12 mesi al prezzo allora in vigore. L'Utente puo':

- Disattivare il rinnovo in qualsiasi momento da Google Play
  (`https://play.google.com/store/account/subscriptions`)
- Continuare a usare l'App fino alla scadenza dell'anno gia' pagato
- Riattivare l'abbonamento in futuro senza penali

Non sono previste penali per cancellazione anticipata.

Gli abbonamenti **Single Lifetime** e **Family Lifetime** **non comportano
alcun rinnovo automatico** ne' addebito periodico: il pagamento e' unico
e l'accesso e' a vita.

### 4.5 Diritto di recesso e rimborsi

#### 4.5.1 Abbonamenti Annual

Per gli abbonamenti **Single Annual** e **Family Annual** le richieste di
rimborso sono gestite secondo la policy di Google Play Store:

- **Entro 48 ore** dall'acquisto: refund automatico tramite Google Play
  (`Account > Cronologia ordini > Richiedi rimborso`)
- **Oltre 48 ore**: refund solo a discrezione di Google. PluriFin puo'
  inoltrare la richiesta a Google ma non ha il controllo della decisione.

L'Utente puo' disattivare il rinnovo in qualsiasi momento senza penali
(sezione 4.4).

#### 4.5.2 Abbonamenti Lifetime e diritto di recesso 14 giorni

Per gli abbonamenti **Single Lifetime** e **Family Lifetime**, in quanto
contenuti digitali forniti su supporto non materiale ai sensi dell'art.
59 comma 1 lett. o) del Codice del Consumo (D.Lgs. 206/2005):

- L'acquisto avviene tramite Google Play Billing con sblocco **immediato**
  delle funzionalita' Premium. L'Utente, completando il pagamento, accetta
  espressamente l'esecuzione immediata del contratto e prende atto della
  conseguente perdita del diritto di recesso ai sensi del combinato
  disposto degli artt. 52 e 59 comma 1 lett. o) del Codice del Consumo.
- Nonostante quanto precede, **PluriFin si impegna a onorare in via
  commerciale il diritto di recesso ex art. 52 del Codice del Consumo
  per 14 giorni** dall'acquisto, su richiesta scritta del consumatore
  inviata a `legal@plurifin.app`. La richiesta deve indicare
  l'order ID Google Play e l'account Google associato all'acquisto.
- Su tali richieste PluriFin inoltrera' la pratica di rimborso a Google
  e, in caso di diniego da parte di Google, riconoscera' direttamente al
  consumatore un credito di valore equivalente tramite gift code Google
  Play o, in subordine, un rimborso bancario equivalente.

Decorsi i 14 giorni dall'acquisto, eventuali richieste di rimborso sono
soggette esclusivamente alla discrezione di Google Play.

#### 4.5.3 Posizione di PluriFin

PluriFin non emette refund diretti su carta in quanto non riceve
direttamente il pagamento dell'Utente: il refund tecnico avviene sempre
attraverso il canale Google Play. La garanzia 14 giorni di cui alla
sezione 4.5.2 costituisce un impegno **commerciale supplementare** di
PluriFin nei confronti del consumatore, che non sostituisce ne' limita
i diritti non derogabili riconosciuti dal Codice del Consumo.

### 4.6 Family plan -- ruoli e responsabilita'

Per gli abbonamenti **Premium Family** (Annual e Lifetime) si applicano
le seguenti regole specifiche.

#### 4.6.1 Parte contraente

L'unico **contraente** del contratto Family Premium e' l'Utente che
effettua l'acquisto via Google Play (di seguito "Account Holder"), in
quanto pagatore. L'Account Holder accetta i presenti Termini in nome e
per conto proprio e si impegna a renderne edotti i propri Family Members
(definiti sotto). I Family Members beneficiano dell'unlock Premium ma
non sono parti contraenti del contratto di subscription con PluriFin.

#### 4.6.2 Composizione del Family Group

L'abilitazione Family copre l'Account Holder e fino a **5 ulteriori
Family Members** facenti parte della medesima Google Family Group
configurata dall'Account Holder (totale max 6 utenti, secondo le
limitazioni correnti di Google Family). La composizione del Google
Family Group e' gestita esclusivamente tramite gli strumenti Google
(`https://families.google.com`): PluriFin non interviene su inviti,
accettazioni o rimozioni dei Family Members.

#### 4.6.3 Diritti di privacy individuali

Ogni Family Member resta titolare individuale dei propri diritti ai sensi
del GDPR (Reg. UE 2016/679) e del Codice in materia di protezione dei
dati personali (D.Lgs. 196/2003 e successive modificazioni), in
particolare per quanto riguarda i dati personali trattati dall'App sul
proprio dispositivo (vedi Privacy Policy).

#### 4.6.4 Cessazione degli effetti della membership

L'accesso Premium di un Family Member viene meno automaticamente in caso
di:

- Rimozione del Family Member dal Google Family Group (per decisione
  dell'Account Holder o di Google)
- Cessazione dell'abbonamento Family da parte dell'Account Holder
- Scadenza naturale del periodo Annual senza rinnovo
- Risoluzione del contratto ai sensi della sezione 10

Le suddette cessazioni sono gestite automaticamente da Google Play
Billing senza intervento di PluriFin.

#### 4.6.5 Responsabilita' dell'Account Holder

L'Account Holder e' civilmente responsabile per l'uso dell'App da parte
dei propri Family Members nei limiti previsti dalla normativa applicabile
e, in particolare, e' tenuto a:

- Garantire che i Family Members siano informati dei presenti Termini e
  del Disclaimer Finanziario
- Disabilitare l'accesso Family in caso di uso non autorizzato o di
  cessazione dei rapporti familiari

### 4.7 Variazione prezzo

PluriFin si riserva il diritto di modificare il prezzo degli abbonamenti
**Annual** (Single Annual e Family Annual). In caso di aumento per utenti
gia' abbonati:

- Notifica via app + email (se l'Utente ha fornito email) almeno **30 giorni
  prima** della scadenza dell'abbonamento corrente
- Possibilita' per l'Utente di disattivare il rinnovo senza alcuna penale

Gli abbonamenti **Lifetime** (Single Lifetime e Family Lifetime) non sono
soggetti a variazioni di prezzo dopo l'acquisto, in quanto il pagamento
e' unico e gia' definitivamente eseguito. Eventuali nuove versioni
Premium future non incideranno su Lifetime gia' acquistati.

## 5. Licenza d'uso

PluriFin concede all'Utente una **licenza non esclusiva, non trasferibile,
revocabile in caso di violazione**, per uso personale dell'App.

L'Utente NON puo':

- Decompilare, disassemblare, fare reverse engineering del software (salvo nei
  limiti consentiti dall'art. 64-quater L. 633/1941)
- Rimuovere, alterare o oscurare avvisi, copyright, marchi
- Rivendere, sublicenziare, noleggiare, distribuire l'App
- Utilizzare l'App per scopi illeciti, fraudolenti o lesivi di diritti di terzi
- Aggirare le misure tecniche di protezione (ad es. validazione subscription)
- Usare l'App per fornire servizi a terzi a titolo professionale

L'Utente PUO':

- Usare l'App per le proprie finanze personali e familiari
- Esportare i propri dati e portarli su altre app
- Mantenere copie locali dei propri dati
- Disinstallare in qualsiasi momento

## 6. Account e sicurezza

L'App **non richiede un account** PluriFin. Tutto e' locale al dispositivo.
L'eventuale account Google Play associato all'acquisto della subscription e'
gestito interamente da Google.

L'Utente e' responsabile della sicurezza del proprio dispositivo: lock screen,
backup, smarrimento. PluriFin non puo' recuperare dati di portafoglio se il
dispositivo viene perso e non e' stato fatto un export manuale.

## 7. Limitazione di responsabilita'

### 7.1 Principio generale

L'App e' fornita "AS IS" e "AS AVAILABLE", senza garanzie di alcun tipo eccetto
quelle non escludibili dalla legge applicabile (in particolare il D.Lgs.
206/2005 -- Codice del Consumo per consumatori UE).

### 7.2 Esclusioni espresse

Nei limiti massimi consentiti dalla legge applicabile, PluriFin NON e'
responsabile per:

- Decisioni di investimento prese dall'Utente sulla base dei dati o delle
  analisi mostrati dall'App
- Perdite finanziarie subite dall'Utente, di qualsiasi entita', incluse perdite
  di capitale, mancato guadagno, perdite di opportunita'
- Errori, ritardi, indisponibilita' o inesattezze dei dati di mercato forniti
  da terzi (EODHD, FMP, Google Finance)
- Errori, allucinazioni, dati obsoleti o fuorvianti contenuti nelle risposte
  dei modelli AI (Gemini)
- Errori di parsing dei file CSV/PDF di estratto broker (i formati cambiano
  senza preavviso)
- Indisponibilita' del servizio di backend (Cloudflare Worker) e conseguente
  blocco temporaneo della validazione subscription
- Perdita di dati locali per disinstallazione, malfunzionamento del dispositivo,
  mancato backup da parte dell'Utente
- Compromissione del dispositivo dell'Utente (root, jailbreak, malware)

### 7.3 Limite quantitativo

In ogni caso, e ferme restando le esclusioni di legge per dolo o colpa grave
(art. 1229 c.c.), l'eventuale responsabilita' complessiva di PluriFin verso
l'Utente, per qualsiasi causa, e' limitata al maggiore fra:

- **EUR 50,00** (cinquanta euro)
- L'importo effettivamente pagato dall'Utente per la subscription Premium nei
  12 mesi precedenti l'evento

### 7.4 Diritti del consumatore non escludibili

Nulla nei presenti Termini limita o esclude i diritti dell'Utente consumatore
ai sensi del Codice del Consumo (D.Lgs. 206/2005), in particolare:

- Garanzia legale di conformita' (artt. 128-135-vicies del Codice del Consumo)
- Diritto di recesso ove applicabile (artt. 49 e seguenti del Codice del Consumo,
  con esclusione dei contenuti digitali ai sensi dell'art. 59.1.o se l'Utente
  ha espressamente accettato l'esecuzione immediata)
- Tutela contro clausole vessatorie (artt. 33-37 del Codice del Consumo)

## 8. Servizi e contenuti di terzi

L'App si integra opzionalmente con servizi di terzi. L'Utente accetta che:

- Le analisi AI sono fornite da Google Gemini, soggette ai
  [Google Gemini API Terms](https://ai.google.dev/terms)
- I dati di mercato real-time sono forniti da EODHD e/o FMP, soggetti ai
  rispettivi termini
- Il pagamento avviene via Google Play, soggetto alle
  [Politiche di Google Play](https://play.google.com/about/play-terms/)
- L'hosting di documenti legali e webapp e' su GitHub Pages, soggetto ai
  [GitHub Terms](https://docs.github.com/en/site-policy/github-terms/github-terms-of-service)

PluriFin non controlla questi servizi e non risponde di malfunzionamenti,
modifiche, sospensioni o cessazioni decise da tali terzi.

## 9. Modifica dei Termini

PluriFin puo' modificare i presenti Termini. Le modifiche sostanziali saranno
notificate:

- In app, con schermata di re-consent al primo avvio successivo all'aggiornamento
- Sul sito vetrina con banner per 30 giorni

Se l'Utente non accetta le modifiche puo' disinstallare l'App e richiedere
refund della subscription residua a Google secondo policy Play.

## 10. Risoluzione e cessazione del servizio

### 10.1 Risoluzione da parte dell'Utente

L'Utente puo' risolvere il contratto in qualsiasi momento:

1. Disattivando il rinnovo della subscription da Google Play
2. Disinstallando l'App

Non sono previste penali ne' formalita' aggiuntive.

### 10.2 Risoluzione da parte di PluriFin

PluriFin puo' risolvere il contratto e revocare la licenza in caso di:

- Violazione sostanziale dei presenti Termini da parte dell'Utente
- Uso fraudolento dell'App
- Tentativi di aggiramento delle protezioni anti-tamper / anti-piracy

In caso di risoluzione per causa imputabile all'Utente, non e' previsto refund
proporzionale.

### 10.3 Cessazione del servizio da parte di PluriFin

PluriFin si riserva il diritto di cessare lo sviluppo o la disponibilita'
dell'App con preavviso minimo di **90 giorni** comunicato:

- In app
- Sul sito vetrina
- Via email agli abbonati Premium

In caso di cessazione, gli abbonamenti gia' pagati saranno onorati fino alla
naturale scadenza, oppure rimborsati pro-quota tramite Google Play.

## 11. Marchi e proprieta' intellettuale

- Il marchio commerciale "PluriFin" e' di Filippo Salemi (deposito EUIPO
  rimandato a Fase 2 post-revenue, vedi master plan)
- Il nome dell'app "Portfolio Manager" e' descrittivo, non oggetto di
  registrazione
- Loghi, icone, font, illustrazioni, codice sorgente sono protetti da diritto
  d'autore (L. 633/1941)
- Marchi di terzi (Google, Android, EODHD, FMP, Cloudflare, GitHub, ecc.) sono
  di proprieta' dei rispettivi titolari e usati solo per descrivere
  l'integrazione

L'Utente non acquisisce alcun diritto su marchi, loghi o codice di PluriFin.

## 12. Protezione dei dati personali

Il trattamento dei dati personali dell'Utente e' disciplinato dalla **Privacy
Policy** (`legal/privacy_policy.md`), parte integrante dei presenti Termini.

## 13. Disclaimer finanziario

Il **Disclaimer Finanziario** (`legal/disclaimer_financial.md`) e' parte
integrante dei presenti Termini e deve essere letto e accettato dall'Utente
prima dell'uso dell'App.

## 14. Foro competente e legge applicabile

I presenti Termini sono regolati dalla **legge italiana**.

Per controversie con l'Utente consumatore, il foro competente e' quello del
luogo di residenza o domicilio elettivo del consumatore ai sensi dell'art. 66-bis
del Codice del Consumo (D.Lgs. 206/2005), salva diversa scelta consentita dalla
legge.

Per controversie con utenti non consumatori (uso professionale), il foro
competente esclusivo e' quello di **Milano (Italia)**, ai sensi degli artt.
18 e 20 c.p.c. quale foro del titolare con sede in Italia.

## 15. Risoluzione alternativa delle controversie (ADR)

L'Utente consumatore puo' ricorrere alla piattaforma ODR della Commissione
Europea: <https://ec.europa.eu/consumers/odr/>

## 16. Clausola di salvaguardia

Se una clausola dei presenti Termini fosse dichiarata invalida o inefficace
da un'autorita' competente, le restanti clausole resteranno valide ed
efficaci.

## 17. Comunicazioni

Le comunicazioni di PluriFin all'Utente avvengono:

- In app (notifica in-app, schermata di consent, banner)
- Email se l'Utente ha fornito un indirizzo
- Sul sito vetrina (banner pubblico)

Le comunicazioni dell'Utente a PluriFin avvengono via email all'indirizzo
appropriato (vedi sezione 12 della Privacy Policy).

## 18. Versionamento

Gli aggiornamenti del presente documento sono identificati dal numero di
versione in alto. La history completa e' disponibile nel repo pubblico
`lunapiena49/portfoliomanager-data` cartella `legal/`.
